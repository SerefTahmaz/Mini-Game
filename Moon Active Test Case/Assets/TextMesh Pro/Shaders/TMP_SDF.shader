...
SubShader {
	...
	Pass {
		...
		struct pixel_t {
			...
			float4	texcoord3		: TEXCOORD4;		// u,v, scale, bias
			fixed4	shadowColor		: COLOR1;
		#endif
		...
		pixel_t VertShader(vertex_t input)
		{
			...
			float bScale2 = scale;
			bScale2 /= 1 + ((_ShadowSoftness*_ScaleRatioD) * bScale2);
			float bBias2 = (0.5 - weight) * bScale2 - 0.5 - ((_ShadowDilate * _ScaleRatioD) * 0.5 * bScale2);
			...
			float y3 = -(_ShadowParentOffsetY) * _GradientScale / _TextureWidth;
		#if (UNDERLAY_ON || UNDERLAY_INNER)
			bOffset.x += x3;
			bOffset.y += y3;
		#endif
...
#if (SHADOW_ON || SHADOW_INNER)
			output.atlas = (input.texcoord0 + float2(x3,y3));
#else
		...
		fixed4 PixShader(pixel_t input) : SV_Target
		{
			...
			faceColor += input.shadowColor * saturate(d2 - input.texcoord3.w) * (1 - faceColor.a);
		#endif

		#if SHADOW_INNER
			float d2 = tex2D(_MainTex, input.texcoord3.xy).a * input.texcoord3.z;
			faceColor += input.shadowColor * (1 - saturate(d2 - input.texcoord3.w)) * saturate(1 - sd) * (1 - faceColor.a);
		#endif
